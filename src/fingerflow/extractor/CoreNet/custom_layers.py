...
def yolov4_head(yolo_neck_outputs):
    anchors = np.array(constants.ANCHORS).reshape((3, 3, 2))

    # Wrap get_boxes calls in Lambda layers for Keras 3.x compatibility
    bbox0, object_probability0, class_probabilities0, pred_box0 = layers.Lambda(
        lambda x: get_boxes_tf(x, anchors[0, :, :], 52, 8, constants.XY_SCALE[0])
    )(yolo_neck_outputs[0])
    
    bbox1, object_probability1, class_probabilities1, pred_box1 = layers.Lambda(
        lambda x: get_boxes_tf(x, anchors[1, :, :], 26, 16, constants.XY_SCALE[1])
    )(yolo_neck_outputs[1])
    
    bbox2, object_probability2, class_probabilities2, pred_box2 = layers.Lambda(
        lambda x: get_boxes_tf(x, anchors[2, :, :], 13, 32, constants.XY_SCALE[2])
    )(yolo_neck_outputs[2])
...
def yolov4_neck(x):
    ...
    route0 = x
    x = conv(x, 256, 3)
    conv_sbbox = conv(x, 3 * (constants.NUM_CLASSES + 5), 1, activation=None, batch_norm=False)
    ...
    route1 = x
    x = conv(x, 512, 3)
    conv_mbbox = conv(x, 3 * (constants.NUM_CLASSES + 5), 1, activation=None, batch_norm=False)
    ...
    x = conv(x, 1024, 3)
    conv_lbbox = conv(x, 3 * (constants.NUM_CLASSES + 5), 1, activation=None, batch_norm=False)

    return [conv_sbbox, conv_mbbox, conv_lbbox]


def cspdarknet53(input_layer):
    x = conv(input_layer, 32, 3)
    ...
    x = conv(x, 1024, 3)
    route2 = conv(x, 512, 1)
    return models.Model(input_layer, [route0, route1, route2])
...
def get_boxes_tf(pred, anchors, grid_size, strides, xyscale):
...
def conv(x, filters, kernel_size, downsampling=False, activation='leaky', batch_norm=True):
    # Define mish activation as a proper function for Keras 3.x compatibility
    def mish_activation(x):
        return layers.Lambda(lambda x: x * tf.math.tanh(tf.math.softplus(x)))(x)
...
def nms(model_outputs):
    """
    Apply Non-Maximum suppression - Keras 3.x compatible wrapper
    """
    return layers.Lambda(nms_tf)(model_outputs)


def nms_tf(model_ouputs):
    """
